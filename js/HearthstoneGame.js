...
class HearthstoneGame {
    constructor() {
        ...
        this.isAITurn = false;
        this.aiThinkingTime = 500; // AI思考时间(毫秒)
    ...
    showChooseClassModal() {
        ...
        confirmBtn.onclick = function () {
            ...
            if (step === 0) {
                ...
                // 修复：每次选择都重置实例的职业属性
                if (window._game) {
                    window._game.playerClass = selectedPlayerClass;
                    window._game.enemyClass = selectedEnemyClass;
                }
    ...
    initializeDecks() {
        ...
        // 全局卡组上限 30 张
        const allConfigs = [
            ...minionConfigs.map(cfg => ({ ...cfg, type: 'minion' })),
            ...weaponConfigs.map(cfg => ({ ...cfg, type: 'weapon' })),
            ...spellConfigs.map(cfg => ({ ...cfg, type: 'spell' }))
        ];
        // 每种卡最多2张，卡组上限30张，不足可少
        const getLimitedDeck = () => {
            let pool = [];
            allConfigs.forEach(cfg => {
                if (!cfg.isToken && !cfg.isSummon) {
                    pool.push(cfg);
                    pool.push(cfg);
                }
            });
            pool = GameUtils.shuffleArray(pool);
            // 判断是否为关键词冒险模式
            const isKeywordMode = this.gameMode === 'vs-ai' && this.adventureMode && this.adventureMode !== 'random';
            let selected;
            if (isKeywordMode) {
                // 只用全部可用卡（不足30张不补足）
                selected = pool;
            ...
            return selected.map(cfg => {
                if (cfg.type === 'minion') return GameUtils.createMinion(cfg, this.minionIdCounter++);
                if (cfg.type === 'weapon') return GameUtils.createWeapon(cfg, this.minionIdCounter++);
                if (cfg.type === 'spell') return GameUtils.createSpell(cfg, this.minionIdCounter++);
            });
        };
        this.player.deck = getLimitedDeck();
        this.enemy.deck = getLimitedDeck();
